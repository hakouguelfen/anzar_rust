...
pub async fn login(
    ...
    if let None = user_detail {
        return Err(HttpResponse::Unauthorized().json("User not found"));
    }
    ...
    if !is_valid {
        return Err(HttpResponse::Unauthorized().json("password wrong"));
    }
...
pub async fn register(
    ...
    if let Err(_) = user_detail {
        return Err(HttpResponse::Unauthorized().json("User creation error"));
    }
...
pub async fn refresh_user_token(
    ...
    if let Err(_) = user_id {
        return Err(HttpResponse::Unauthorized().json("Error parsing objectId"));
    }
    ...
    .unwrap_or(false);
    if !is_valid {
        return Err(HttpResponse::Unauthorized().json("comapre token failure"));
    }
...
pub async fn sync_token(
    ...
    if let Err(_) = tokens_response {
        return Err(HttpResponse::Unauthorized().json("Token creation error"));
    }
